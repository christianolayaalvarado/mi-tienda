...
export default function ScrollMascot() {
  ...
  // Primer mensaje: Saludo con nombre
  useEffect(() => {
  ...
  // Segundo mensaje: al hacer un poco de scroll
  useEffect(() => {
    if (progress > 0.03 && progress < 0.15 && lastMessageZone.current === "") {
      lastMessageZone.current = "scrolled";
      setMessage("¡Hay más productos abajo! Sigue bajando 👇");
      setMsgKey((k) => k + 1);
    }
  }, [progress]);
  ...
  useEffect(() => {
    ...
    if (progress < 0.03) {
      ...
      zone = "early";
      msg = "¡Hay más productos abajo! Sigue bajando 👇";
  ...
  // Posición: más abajo, empieza en 120px
  const viewH = typeof window !== "undefined" ? window.innerHeight : 800;
  const mascotTop = 120 + progress * (viewH - 220);
      ...
      >
        ...
        <div className={`transition-transform duration-200 ${isWaving || isJumping ? "animate-[wiggle_0.4s_ease-in-out_3]" : "group-hover:scale-110"}`}>
          <svg width="44" height="56" viewBox="0 0 64 80" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-lg">
            ...
            {/* Brazos */}
            <rect x="4" y="30" width="6" height="18" rx="3" fill="#D97706" style={{ transformOrigin: "7px 30px", transform: `rotate(${isWaving ? armAngle : 0}deg)`, transition: "transform 0.3s ease" }} />
            <rect x="54" y="30" width="6" height="18" rx="3" fill="#D97706" style={{ transformOrigin: "57px 30px", transform: `rotate(${isWaving ? -armAngle * 0.8 : 0}deg)`, transition: "transform 0.3s ease" }} />
