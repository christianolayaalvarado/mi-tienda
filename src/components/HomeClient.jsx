...
export default function HomeClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 py-8 pb-20 sm:pb-16">
      ...
      {/* Banner Celebración */}
      {celebration && !currentSearch && !currentCategory && (
        <a href={celebration.bannerLink} className="block mb-4 rounded-xl overflow-hidden relative group">
          <div className="w-full h-[100px] sm:h-[120px] flex items-center justify-between px-5 sm:px-8"
            style={{ background: celebration.bannerGradient }}>
            <div className="flex items-center gap-3 sm:gap-4">
              <img src={celebration.bannerImage} alt={celebration.name} width="50" height="50" className="shrink-0 drop-shadow-lg" />
              <div>
                <p className="text-lg sm:text-2xl font-extrabold drop-shadow-lg tracking-wide" style={{ color: celebration.bannerTextColor }}>{celebration.bannerTitle}</p>
                <p className="text-xs sm:text-sm font-semibold" style={{ color: celebration.bannerTextColor, opacity: 0.9 }}>{celebration.bannerSubtitle}</p>
              </div>
            </div>
            <div className="hidden sm:flex items-center gap-2">
              <span className="bg-white text-sm font-bold px-4 py-2 rounded-full group-hover:scale-105 transition-transform" style={{ color: celebration.bannerTextColor === "#fff" ? "#C8102E" : "#333" }}>
                Ver ofertas →
              </span>
            </div>
          </div>
